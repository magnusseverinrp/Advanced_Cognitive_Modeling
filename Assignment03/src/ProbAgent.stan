...
model {
  ...
  vector[N] beta_post  = 0.5 + p * (to_vector(total_1) - to_vector(rating_1))
...
generated quantities {
  ...
  for (i in 1:N) {
    real alpha_post = 0.5 + p * rating_1[i] + (1.0 - p) * rating_g[i];
    real beta_post  = 0.5 + p * (total_1[i] - rating_1[i])
                          + (1.0 - p) * (total_g[i] - rating_g[i]);

    log_lik[i]        = beta_binomial_lpmf(choice[i] | 1, alpha_post, beta_post);
    posterior_pred[i] = beta_binomial_rng(1, alpha_post, beta_post);

    real ap = 0.5 + p_prior * rating_1[i] + (1.0 - p_prior) * rating_g[i];
    real bp = 0.5 + p_prior * (total_1[i] - rating_1[i])
                  + (1.0 - p_prior) * (total_g[i] - rating_g[i]);
