...
data {
  ...
  array[N_total] int<lower=0, upper=1> danger_feedback;
...
transformed parameters {
  ...
  vector<lower=1e-9, upper=1-1e-9>[N_total] prob_danger1;

  {
    array[N_total] int memory_trial_idx;
    array[N_total] int memory_danger;
    int n_mem = 0;
    ...
    for (i in 1:N_total) {
      ...
      for (k in 1:n_mem) {
        if (memory_danger[k] == 0) has_danger0 = 1;
        if (memory_danger[k] == 1) has_danger1 = 1;
      }

      if (n_mem == 0 || has_danger0 == 0 || has_danger1 == 0) {
        p_i = bias;
      } else {
        real s1 = 0; real s0 = 0;
        int n1 = 0;  int n0 = 0;
        for (e in 1:n_mem) {
          ...
          real sim = exp(-c * d);
          if (memory_danger[e] == 1) { s1 += sim; n1 += 1; }
          else                    { s0 += sim; n0 += 1; }
        }
        real m1 = s1 / n1;
        real m0 = s0 / n0;
      ...
      prob_danger1[i] = fmax(1e-9, fmin(1 - 1e-9, p_i));

      n_mem += 1;
      memory_trial_idx[n_mem] = i;
      memory_danger[n_mem] = danger_feedback[i];
    }
  }
}
...
model {
  ...
  target += bernoulli_lpmf(y | prob_danger1);
}
...
generated quantities {
  ...
  if (run_diagnostics) {
    for (i in 1:N_total)
      log_lik[i] = bernoulli_lpmf(y[i] | prob_danger1[i]);
  }
