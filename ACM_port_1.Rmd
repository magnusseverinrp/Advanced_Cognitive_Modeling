...
rl_agent_f <- function(estimated_value, feedback, alpha = 1) { 
  ...
  estimated_value <- estimated_value + alpha * (feedback - estimated_value)
  
  choice <- ifelse(estimated_value >= 0.5, 1, 0) 
...
for (t in 2:trials) {
  ...
  result <- rl_agent_f(estimated_value_tracked[t - 1], prev_feedback2) #WSLS agent choice
  RL_choices[t] <- result$choice #saving WSLS agent choice
  estimated_value_tracked[t] <- result$estimated_value #saving WSLS agent estimated value
...
df_WSLS_RL <- tibble(
  ...
  Feedback = feedback_wsls, # 1 if Self_WSLS won
