...
```{r}
rl_agent_f <- function(last_choice, estimated_value, feedback, alpha = 0.5, beta=1) { 
  k <- last_choice + 1 
...
df <- map_dfr(alpha_values, full_simulation) #run full simulation for all alpha values appending each                                     df_WSLS_RL to create df containing simulations for each alpha value
```
...
p_choices_ev <- ggplot(df %>% filter(alpha == 0.4), aes(x = trial)) +
  ...
  labs(title = "Reinforcement Learning - Estimated Value", y = "Estimated Value") + theme_classic() + ylim(-0.1, 1.1)
